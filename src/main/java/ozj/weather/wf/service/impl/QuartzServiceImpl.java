...
public class QuartzServiceImpl implements QuartzService {
     ...
     */
    @Override
    public void addJob(Class clazz, String name, String groupName, String cronExp, Map<String, String> param) {
        try {
            // 启动调度器
            scheduler.start();
            //构建job信息
            JobDetail jobDetail = JobBuilder.newJob(((Job) clazz.newInstance()).getClass()).withIdentity(name, groupName).withDescription(param.get("description")).build();
            //表达式调度构建器(即任务执行的时间)
